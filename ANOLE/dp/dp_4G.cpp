...
float DT = 0.3;  // time granularity
float BUFFER_THRESH = 60.0 * MILLISECONDS_IN_SECOND;  // sec, max buffer limit
static const int VIDEO_BIT_RATE[] = {200, 800, 2200, 5000, 10000,18000,32000,50000}; // Kbps
unsigned int DEFAULT_QUALITY = 1;
unsigned int MAX_QUALITY = 7;
float REBUF_PENALTY = 30;// 1 sec rebuffering -> 3 Mbps
float REBUF_PENALTY_LOG = 5.23;
int SMOOTH_PENALTY = 1;
float INVALID_DOWNLOAD_TIME = -1;
std::string COOKED_TRACE_FOLDER = "../data/test/";
std::string OUTPUT_FILE_PATH = "../results_up_bound/";
std::string RESULTS_FILE_NAME = "sim_dp";
...
ALL_COOKED_TIME_BW get_all_cooked_time_bw(std::string path, std::string number) {
	...
	if (dir != NULL) {
		while ((entry = readdir(dir)) != NULL) {
			...
			file_name += COOKED_TRACE_FOLDER+ number+"/";
			...
			if (in_file.is_open())
			{
				...
				while ( getline (in_file, line) )
				{
					...
					//printf("%f\t%f\n",std::stof(parse[0]),std::stof(parse[1]));
				}
				in_file.close();
			}
...
std::vector<std::vector<unsigned int> > get_video_sizes() {
	...
	for(int bit_rate = 0; bit_rate < BITRATE_LEVELS; bit_rate ++) {
        ...
        int seg_number =0;
        for( seg_number =0 ; seg_number<TOTAL_VIDEO_CHUNCK; seg_number++){

        video_size.push_back(static_cast<int>(VIDEO_BIT_RATE[bit_rate] * 500));

        }
        //printf("%d:%d\n",video_size[seg_number-1],seg_number);
...
int main(int argc, char *argv[]) {
    ...
    std::cout<<arg1<<" "<<arg2<<"\n";
	ALL_COOKED_TIME_BW all_cooked_time_bw = \
		get_all_cooked_time_bw(COOKED_TRACE_FOLDER + arg1 + "/",arg1);
	std::vector<std::vector<unsigned int> > video_sizes = \
		get_video_sizes();
    ...
    if(arg2 == "qoe1"){
        output_path = OUTPUT_FILE_PATH + "QoE1/" + arg1 +"/";
        int status = mkdir((output_path ).c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    }
    else if(arg2 == "qoe2"){
        output_path = OUTPUT_FILE_PATH + "QoE2/" + arg1 +"/";
        int status = mkdir((output_path ).c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    }
    else {
    printf("第二个参数错误!!!!!\n\n\n\n");
	...
	for (unsigned int trace_idx = 0;
		...
		log_file.open (output_path + RESULTS_FILE_NAME + "_" + all_cooked_time_bw.all_file_names[trace_idx]);
        ...
        float reward = 0;
        if(arg2 == "qoe1"){
            reward = VIDEO_BIT_RATE[DEFAULT_QUALITY] / M_IN_K \
			- REBUF_PENALTY * chunk_finish_time;
        }

        else if(arg2 == "qoe2"){

            float log_bit_rate = std::log(VIDEO_BIT_RATE[DEFAULT_QUALITY] / float(VIDEO_BIT_RATE[0]));
            reward = log_bit_rate \
                            -REBUF_PENALTY_LOG *chunk_finish_time;
        }
		...
		for (unsigned int n = 1; n < TOTAL_VIDEO_CHUNCK; n ++) {
            ...
            printf("%lu\n\n", std::distance(total_reward[n-1].begin(), total_reward[n-1].end()));
				...
				for (unsigned int nm = 0; nm < BITRATE_LEVELS; nm ++) {
					...
					if (rebuf < 0) rebuf = 0;
                     ...
                     if(arg2 == "qoe1"){
                        ...
                        float log_bit_rate = std::log(VIDEO_BIT_RATE[nm] / float(VIDEO_BIT_RATE[0])) ;
                        float log_last_bit_rate = std::log(VIDEO_BIT_RATE[m] / float(VIDEO_BIT_RATE[0]));
                        reward = log_bit_rate \
                            - REBUF_PENALTY_LOG * rebuf \
                            - SMOOTH_PENALTY * abs(log_bit_rate - log_last_bit_rate);
                     }
		...
		for (auto it = total_reward[TOTAL_VIDEO_CHUNCK-1].begin(); it != total_reward[TOTAL_VIDEO_CHUNCK-1].end(); it ++) {
			...
			printf("Key: %lu, t: %u, b: %u, m: %u, tmp_total_reward: %f\n", key, t, b, m, tmp_total_reward);
			//if(t>=t_max_idx){
			  //  t=t-100;
			//}
            //printf(" t: %d\n", t);
