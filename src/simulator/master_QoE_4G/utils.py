...
from simulator.master_QoE_4G.constants import (HD_REWARD, M_IN_K, VIDEO_BIT_RATE)
...
def QoE_5(current_bitrate: int, last_bitrate: int, broad_rebuf: float, delay: float,get_vidro_delay:float,broad_buffer_delay:float):
    """ Return linear QoE metric.
        侧重于延迟QoE
        """
...
def QoE_6(current_bitrate: int,  delay: float, get_vidro_delay:float):
    """ Return linear QoE metric.
        侧重于识别率的QoE
        """
